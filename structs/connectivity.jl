...
function check_and_report_connections_bfs(g::MGraph, source_net_sets::Vector{Tuple{String, Set{String}}}, libname::String, cellname::String)::Vector{ComponentInfo}
                ...
                if current_netname !== nothing
                    ...
                    elseif current_netname != expected_netname_ref[] && expected_netname_ref[] !== nothing
                        if component_consistent[] # 첫 불일치 시 로그
                             node_id_str = hasproperty(u_node, :idx) ? " (idx=$(u_node.idx))" : ""
                             @warn "  SHORT! : Node$(node_id_str) has netname '$current_netname', but expected '$(expected_netname_ref[])' for this component.
                             $(u_node.netname) : layer=$(u_node.layer), p_coord=$(u_node.p_coord), s_coord=$(u_node.points[1].s_coord) - $(u_node.points[2].s_coord)
                             $(start_node.netname) : layer=$(start_node.layer), p_coord=$(start_node.p_coord), s_coord=$(start_node.points[1].s_coord) - $(start_node.points[2].s_coord)"
                            error_cnt["short"] += 1; error_cnt["total"] += 1;
                        end
    ...
    for (i, component) in enumerate(all_components_info)
        ...
        println("Representative Net Name: $netname_str")

        # 일관성 여부 출력
        println("Netname Consistency: $(component.is_consistent ? "Passed" : "Failed")")
        ...
        if !isempty(component.nodes)
            ...
            if all(hasproperty(node, :idx) for node in component.nodes)
                 ...
                 println("Metal Indices ($(length(metal_indices))): $metal_indices")
                #  for node in component.nodes
                #     println("$(node.netname) : layer=$(node.layer), p_coord=$(node.p_coord), s_coord=$(node.points[1].s_coord) - $(node.points[2].s_coord)")
                #  end
            else
                 println("Metals ($(length(component.nodes))): [Cannot list indices, some nodes lack .idx property]")
                 # println("Metals: $(component.nodes)") # 전체 객체 출력 (길 수 있음)
            end
        else
            println("Metals: (Empty component)")
        end
    end

    print_consistency_status(libname, cellname, overall_consistent, error_cnt)
...
function print_consistency_status(libname::String, cellname::String, overall_consistent::Bool, error_cnt::Dict{String, Int})
    println("\n------------------------------------------------------------------\n")

    println("Library: $libname")
    println("Cell: $cellname\n")
...
(__...--'' | |-.   |/    _ | /    _ |  |  .---',`'--..._) 
 ...
 `--'      `--' `--' `-----'  `-----'  `------'`-------'    
        """)
        println("Overall Graph Netname Consistency: PASSED ✔️")
    else
        println("""
            ('-.                          ('-.  _ .-') _   
           ( OO ).-.                    _(  OO)( (  OO) )  
...
|_)|  .--' |  .-.  | ,|  |_.'(|  '---.' |  .--' |  |   / : 
  ||  |    |  | |  |(_|  |    |      |  |  `---.|  '--'  / 
   `--'    `--' `--'  `--'    `------'  `------'`-------'  
        """)
        println("Overall Graph Netname Consistency: FAILED ❌")
    end
