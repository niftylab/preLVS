...
cell_list = []
I0 = np.empty(shape=(row, col),dtype=object)
for column_num in range(row): # row, col are equal to number of column, size of column, each
   for stack_num in range(col):
      idx = column_num*col + stack_num
      _netmap = {"SCAN_IN":"SCAN_INT"+str(idx-1), "SCAN_CLK_OUT":"SCAN_CLK_INT"+str(idx-1), "SCAN_DATA_IN":"SCAN_DATA_IN<"+str(idx) +'>',\
                     "SCAN_LOAD":"SCAN_LOAD","SCAN_EN":"SCAN_EN","VSS":"VSS:","VDD":"VDD:",\
                           "SCAN_OUT":"SCAN_INT"+str(idx), "SCAN_DATA_OUT":"SCAN_DATA_OUT<"+str(idx) +'>', "SCAN_CLK":"SCAN_CLK_INT"+str(idx), "SCAN_GATE":"SCAN_GATE", "SCAN_GATE_VALUE":"SCAN_GATE_VALUE<"+str(idx)+'>'}
      if idx == 0:
         _netmap["SCAN_IN"] = "SCAN_IN"
         _netmap['SCAN_CLK_OUT'] = "SCAN_CLK_OUT"
      elif idx+1 > bit:
         for pin in _netmap.keys():
            _netmap[pin] = "DUMMY_"+pin+"_"+str(idx)
            _netmap["VDD"] = "VDD:"
            _netmap["VSS"] = "VSS:"
      elif idx+1 == bit:
         _netmap["SCAN_OUT"] = "SCAN_OUT"
         _netmap['SCAN_CLK'] = "SCAN_CLK"
      I0[column_num,stack_num] = tlib['scan_cell'].generate(name='I0_'+str(idx),netmap=_netmap)
#I0  = tlib[     'scan_cell'  ].generate(name='I0', shape=[row, col])
DMY = templates['ntap_fast_left'].generate(name='DMY')
#dsn.place(grid=pg, inst=np.transpose(I0), mn=[0,0]+pg.mn.width_vec(DMY))

for i in range(len(I0)):
  cursor = [0,0] + pg.mn.width_vec(I0[0][0])*i
  for j in range(len(I0[0])):
    dsn.place(grid=pg, inst=I0[i][j], mn=cursor)
    cursor += pg.mn.height_vec(I0[i][j])
...
for row_idx in range(full_row):
   ...
   _mn = [r45.mn(I0[row_idx,0].pins['SCAN_GATE_M4'])[1], r45.mn(I0[row_idx,-1].pins['SCAN_GATE_M4'])[1]]
   dsn.route(grid=r45, mn=_mn)

   for col_idx in range(col):
      dsn.via(grid=r45, mn=r45.mn(I0[row_idx,col_idx].pins['SCAN_GATE_M4'])[1])
...
for row_idx in range(full_row, row):
   ...
   _mn = [r45.mn(I0[row_idx,0].pins['SCAN_GATE_M4'])[1], r45.mn(I0[row_idx,cells_in_rest_col-1].pins['SCAN_GATE_M4'])[1]]
   dsn.route(grid=r45, mn=_mn)
...
for row_idx in range(row-1): # iteration : row-1 times
   ...
   _metal_track = dsn.route_via_track(grid=r34, mn=_mn, track=_track)
...
for row_idx in range(full_row):
   for col_idx in reversed(range(col-1)): # reversed index
      _mn = [r34.mn(I0[row_idx, col_idx+1].pins['SCAN_CLK_OUT'])[0]+[0,-2], r34.mn(I0[row_idx, col_idx].pins['SCAN_CLK'])[1]]
...
for row_idx in range(full_row):
   for col_idx in range(col):
      ...
      end_pt_y = r45.mn.top_right(I0[row_idx,-1])[1]
      end_pt = np.asarray([end_pt_x, end_pt_y])
      #_mn = np.asarray([r45.mn(I0[row_idx, col_idx].pins['SCAN_DATA_IN'])[1]+[2+col_idx,0], end_pt])  # to fix lvs errors in intel16
      _mn = np.asarray([r45.mn(I0[row_idx, col_idx].pins['SCAN_DATA_IN'])[1]+[2+col_idx,-1], end_pt])
      vertical = dsn.route(grid=r45, mn=_mn, via_tag=[True, False])
      #_mn = np.asarray([r34.mn(I0[row_idx, col_idx].pins['SCAN_DATA_IN'])[1], r34.mn(vertical[0])[0]]) # to fix lvs errors in intel16
      _mn = np.asarray([r34.mn(I0[row_idx, col_idx].pins['SCAN_DATA_IN'])[1]+[0, -1], r34.mn(vertical[0])[0]])
      dsn.route(grid=r34, mn=_mn, via_tag=[True, False])
...
for row_idx in range(full_row, row):
   for col_idx in range(col-1):
      end_pt_x = r45.mn(I0[row_idx, col_idx].pins['SCAN_DATA_IN'])[0,0]+(2+col_idx)
      end_pt_y = r45.mn.top_right(I0[row_idx,-1])[1] - 1
      end_pt = np.asarray([end_pt_x, end_pt_y])
      #_mn = np.asarray([r45.mn(I0[row_idx, col_idx].pins['SCAN_DATA_IN'])[1]+[2+col_idx,0], end_pt])  # to fix lvs errors in intel16
      _mn = np.asarray([r45.mn(I0[row_idx, col_idx].pins['SCAN_DATA_IN'])[1]+[2+col_idx,-1], end_pt])
      vertical = dsn.route(grid=r45, mn=_mn, via_tag=[True, False])
...
for row_idx in range(full_row):
   for col_idx in range(col):
      ...
      end_pt_y = r45.mn.top_right(I0[row_idx,-1])[1]
      end_pt = np.asarray([end_pt_x, end_pt_y])
      #_mn = np.asarray([np.mean(r45.mn(I0[row_idx, col_idx].pins['SCAN_GATE_VALUE']), axis=0, dtype=int)+[0,3]+[-2-col_idx,0], end_pt]) # to fix lvs
      _mn = np.asarray([r45.mn(I0[row_idx, col_idx].pins['SCAN_GATE_VALUE'])[0]+[-2-col_idx,0], end_pt]) 
      _mn[0,1] = np.mean(r45.mn(I0[row_idx, col_idx].pins['SCAN_GATE_VALUE'])[:,1])+3
      vertical = dsn.route(grid=r45, mn=_mn, via_tag=[True, False])
...
# nMap = NetMap.import_from_design(dsn, grid_table, via_table, orient_first="horizontal", layer_names=['M2','M3','M4','M5'], net_ignore = [], lib_ref = "laygo2_generators_private/prj_db/library.yaml")
# metal_num = nMap.count_metals()
# db = nMap.export_all("laygo2_generators_private/scan/scan_chain"+str(row*col)+"_flat.json")
