...
celltype_list = ['outmux_inv', 'outmux_inv_nrtrackswap'] #, 'time_comp_inv_rtrackswap']
...
for celltype in celltype_list:
    for nf in nf_list:
        ...
        pmos_params = {'nf': nf, 'tie': 'S' , 'nfdmyl' : 2}
        nmos_params = {'nf': nf, 'tie': 'S' , 'nfdmyl' : 2}
        if celltype.endswith('nrtrackswap'):
            nmos_params['rtrackswap'] = True
        elif celltype.endswith('rtrackswap'):
            pmos_params['rtrackswap'] = True
            nmos_params['rtrackswap'] = True
        in0 = tnmos.generate(name='MN0',                 params=nmos_params, netmap={'G':'I', 'D':'O', 'RAIL':'VSS:'})
        ip0 = tpmos.generate(name='MP0', transform='MX', params=pmos_params, netmap={'G':'I', 'D':'O', 'RAIL':'VDD:'})
