...
for celltype in cell_type:
        ...
        # core devices 
        in0  = tnmos.generate(name='MN0',                 params={'nf': 6, 'tie': 'S' , 'trackswap' : True })
        in1  = tnmos.generate(name='MN1',                 params={'nf': 6})
        in2  = tnmos.generate(name='MN2',                 params={'nf': 2, 'tie': 'S' , 'bndr' : False})
        
        ip0  = tpmos.generate(name='MP0', transform='MX', params={'nf': 6, 'tie': 'S' , 'bndr' : False, 'nfdmyl' : 8, 'nfdmyr' : 4})
        
        # 4. Place instances
        dsn.place(grid=pg, inst=[in0, in1, in2], mn=[0,0])
        ...
        # NMOS connection
        rncon0 = dsn.route(grid=r23, mn=[in0.p['D'], in1.p['S']])
        
        # DMY vss connection
        _track = [r23(in2.p['G'])[0,0], None]
        vss0 = dsn.route(grid=r12, mn=[in2.p['G'], in2.p['RAIL']], track=_track)

        # OUT
        _mn = [r23.center_right(in1.p['D']), r23.center_right(ip0.p['D'])]
        ...
        _mn = [r12.center_left(in0.p['G']), r23.center_right(in0.p['G'])]
        pgin0 = dsn.pin(name='G_in0', grid=r12, mn=_mn)
        
        _mn = [r12.center_left(in1.p['G']), r23.center_right(in1.p['G'])]
        pgin1 = dsn.pin(name='G_in1', grid=r12, mn=_mn)
        
        _mn = [r12.center_left(ip0.p['G']), r23.center_right(ip0.p['G'])]
        pgip0 = dsn.pin(name='G_ip0', grid=r12, mn=_mn)
