...
const WIRE_HALF_WIDTH = 5 # 와이어 두께 (가정)
...
function path_to_rects(path::Vector{Node})
    if length(path) < 2
        return Rect[]
    end
    ...
    while i < length(path)
        ...
        if start_node.x == end_node.x && start_node.y == end_node.y
            via_type = "via_$(start_node.z)_$(end_node.z)"
            layers = @SVector([start_node.z, end_node.z])
...
function create_grid(layout_bbox::Vector{Int})
    ...
    # M4 트랙 생성 (예시)
    m2_scope = 1200
    m2_elements = [100, 200, 300, 400, 500, 700, 800, 900, 1000, 1100]
    for y_base in 0:m2_scope:(layout_bbox[4])
        for y_el in m2_elements
