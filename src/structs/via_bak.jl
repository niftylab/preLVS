...
function VPoint(
    ...
    return VPoint(xy, extension, layer, type, nothing)
end
...
function json_to_VData(libname::String, cellname::String, json_path::String)::VData
    ...
    for (type, vlist) in json_data
        ...
        for vpoint in vlist["vpoints"]
            push!(vpoints, VPoint(vpoint["xy"], vpoint["extension"], vpoint["layer"], vpoint["type"], nothing))
        end
...
function db_to_VData(libname::String, cellname::String, db_vias::Vector, config_data, perform_sort::Bool=false)
    ...
    for via in db_vias
        ...
        push!(_unsorted_vdata.vlists[_type].vpoints, VPoint(xy=map(Int, via["xy"]), extension=map(Int, config_data["Via"][_type]["extension"]), layer=map(String, via["layer"]), type=_type))
    end

    if perform_sort
        _sorted_vdata = VData(cellname, libname, OrderedDict{String, VList}(), OrderedDict{String, Dict{Int, VList}}(), OrderedDict{String, Dict{Int, VList}}())
...
function transform_VData(vdata::VData, transform::Matrix{Int})
    ...
    for (type, vlist) in vdata.vlists
        ...
        for vpoint in vlist.vpoints
            ...
            push!(new_vlist.vpoints, VPoint(new_xy[1:2], vpoint.extension, vpoint.layer, vpoint.type, nothing))
        end
