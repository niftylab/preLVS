...
function sort_n_merge_MData(mdata::MData)
    ...
    for layer in layer_list
        new_mdata.metals[layer] = MLayer(layer, OrderedDict{Int, Vector{MVector}}())

        # Get all primary coordinates
        p_coords = sort(collect(keys(mdata.metals[layer].metals)))

        for p_coord in p_coords
            new_mdata.metals[layer].metals[p_coord], _named_mvectors = merge_mvector_list(mdata.metals[layer].metals[p_coord], p_coord, layer)
            if length(_named_mvectors) > 0
                append!(named_mvectors, _named_mvectors)
            end
        end
    end
    return new_mdata, named_mvectors
end


function merge_mvector_list(mvector_list::Vector{MVector}, p_coord::Int, layer::Int)::Tuple{Vector{MVector}, Vector{MVector}}
    ...
    for mpoint in mpoints
        ...
        elseif mpoint.pos == END
            ...
            if is_empty_stack(st)
                ...
                push!(merged_metals, MVector(layer, p_coord, SVector{2, MPoint}(MPoint(start_mpoint.s_coord, START, netname), 
    ...
    return merged_metals, named_metal_list
end
